...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-005 | Replace `bg-[var(--background-primary)]` with `bg-surface-primary` in header element | | |
| TASK-006 | Replace `border-[var(--border-color)]` with `border-border-default` in header element | | |
| TASK-007 | Replace `text-[var(--text-secondary)]` with `text-content-secondary` in subtitle span | | |
| TASK-008 | Replace `text-[var(--text-tertiary)]` with `text-content-tertiary` in search icons | | |
| TASK-009 | Replace `bg-[var(--background-secondary)]` with `bg-surface-secondary` in search input wrappers | | |
| TASK-010 | Update mobile menu background from `bg-[var(--background-primary)]` to `bg-surface-primary` | | |
